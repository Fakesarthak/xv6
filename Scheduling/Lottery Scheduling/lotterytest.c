 ...
 */
const long chi_squared_thresholds[] = {
...
int compare_ticks_chi_squared(struct test_case *test) {
    ...
    */
    ...
    int skipped = 0;
    for (int i = 0; i < test->num_children; ++i) {
        long scaled_expected = (test->expect_ticks_unscaled[i] << FIXED_POINT_COUNT) / expect_ticks_total
        ...
        long cur_delta = scaled_expected - (test->actual_ticks[i] << FIXED_POINT_COUNT);
#ifdef DEBUG
    ...
    long expected_value = chi_squared_thresholds[degrees_of_freedom - 1];
...
void compare_ticks_naive(struct test_case *test) {
    ...
    int failed_any = 0;
    for (int i = 0; i < test->num_children; ++i) {
        long scaled_expected = ((long) test->expect_ticks_unscaled[i] * test->total_actual_ticks) / expect_ticks_total;
